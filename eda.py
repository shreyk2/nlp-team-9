...
if __name__ == "__main__":
    ...
    if 4 in task3_data:
        ...
        axes[3].scatter(vecs4_2d[:len(high4), 0], vecs4_2d[:len(high4), 1],
                        c='#ADD8E6', s=60, alpha=0.7, edgecolors='black', linewidth=0.5, label='high (d=4)')
        axes[3].scatter(vecs4_2d[len(high4):, 0], vecs4_2d[len(high4):, 1],
                        c='#FFA07A', s=60, alpha=0.7, edgecolors='black', linewidth=0.5, label='low (d=4)')
    ...
    util_all_means = plot_stability_line(axes[0], utility_stab_all, ns, 'blue', 'utility (all d)', 's')
    util_3_means = plot_stability_line(axes[0], utility_stab_geq3, ns, 'green', 'utility (d>=3)', '^')
    util_4_means = plot_stability_line(axes[0], utility_stab_eq4, ns, 'orange', 'utility (d=4)', 'D')
    ...
    axes[2].set_title(f"Utility Direction Variance: N=32 vs N=max (d>=3)\n(same PCA space - tighter = more stable, layer {EXTRACTION_LAYER})")
    axes[2].legend()
    axes[2].grid(True, alpha=0.3)
    ...
    print(f"{'sample size':>12} | {'safety':^20} | {'util (all d)':^20} | {'util (d>=3)':^20} | {'util (d=4)':^20}")
    print("-" * 97)
    for n in ns:
    ...
    if util_all_cross:
        print(f"utility (all d) stabilizes at N = {util_all_cross}")
    if util_geq3_cross:
        print(f"utility (d>=3) stabilizes at N = {util_geq3_cross}")
    if util_eq4_cross:
